...
patterns = [set(filter(lambda x: len(x)==i+1, patterns.split(", "))) for i in range(len(max(patterns.split(", "), key=len)))]
...
for towel in towels.split("\n"):
    ...
    for i in range(len(towel)):
        count = 0
        for j in range(min(i+1, len(patterns))):
            if towel[i-j:i+1] in patterns[j]:
                count += i==j or ways[i-j-1]
        ways.append(count)
#    if ways[-1] != 0:
#        posible += 1
    posible += ways[-1]
