...
data = """..224.....487...................718.....................378............................................284........310......313..........311.
...
starData = np.array([[1 if j == "*" else 0 for j in i] for i in data.split("\n")])
data = re.sub("[^0-9.\n]", "*", data)
arr = np.array([[(-2 if j == "*" else (-1 if j == "." else int(j))) for j in i] for i in data.split("\n")])
...
for x,y in zip(x,y):
    ...
    if y != 0: # top
        if arr[y-1, x] < 0:
            lenght = np.where((arr[y-1, :x]<=-1)[::-1])[0]
            if lenght.size != 0:
                lenght = lenght[0]
#                if starData[y,x]==0:
 #                   sum += np.sum(arr[y-1, x-lenght:x] * 10**np.arange(lenght)[::-1])
  #              else:
                gears.append(np.sum(arr[y-1, x-lenght:x] * 10**np.arange(lenght)[::-1]))
            else:
#                if starData[y,x]==0:
 #                   sum += np.sum(arr[y-1, :x] * 10**np.arange(arr[y-1, :x].size)[::-1])
  #              else:
                gears.append(np.sum(arr[y-1, :x] * 10**np.arange(arr[y-1, :x].size)[::-1]))
            
            lenght = np.where(arr[y-1,x+1:]<=-1)[0]
            if lenght.size != 0:
                lenght = lenght[0]
#                if starData[y,x]==0:
 #                   sum += np.sum(arr[y-1, x+1:x+1+lenght] * 10**np.arange(lenght)[::-1])
  #              else:
                gears.append(np.sum(arr[y-1, x+1:x+1+lenght] * 10**np.arange(lenght)[::-1]))
            else:
#                if starData[y,x]==0:
 #                   sum += np.sum(arr[y-1, x+1:] * 10**np.arange(arr[y-1, x+1:].size)[::-1])
  #              else:
                gears.append(np.sum(arr[y-1, x+1:] * 10**np.arange(arr[y-1, x+1:].size)[::-1]))
        else:
            left = np.where((arr[y-1,:x]<=-1)[::-1])[0]
            if left.size == 0:
                left = 0
            else:
                left = left[0]
            right = np.where(arr[y-1,x+1:]<=-1)[0]
            if right.size == 0:
                right = 0
            else:
                right = right[0]
            lenght = left + right + 1
#            if starData[y,x]==0:
 #               sum += np.sum(arr[y-1, x-left:x+1+right] * 10**np.arange(lenght)[::-1])
  #          else:
            gears.append(np.sum(arr[y-1, x-left:x+1+right] * 10**np.arange(lenght)[::-1]))
    if x != 0: # left
        lenght = np.where((arr[y,:x]<=-1)[::-1])[0]
        if lenght.size != 0:
            lenght = lenght[0]
#            if starData[y,x]==0:
 #               sum += np.sum(arr[y, x-lenght:x] * 10**np.arange(lenght)[::-1])
  #          else:
            gears.append(np.sum(arr[y, x-lenght:x] * 10**np.arange(lenght)[::-1]))
        else:
#            if starData[y,x]==0:
 #               sum += np.sum(arr[y, :x] * 10**np.arange(arr[y, :x].size)[::-1])
  #          else:
            gears.append(np.sum(arr[y, :x] * 10**np.arange(arr[y, :x].size)[::-1]))
    if y != arr.shape[1]-1: # bottom
        if arr[y+1, x] < 0:
            lenght = np.where((arr[y+1, :x]<=-1)[::-1])[0]
            if lenght.size != 0:
                lenght = lenght[0]
...
#                if starData[y,x]==0:
 #                   sum += np.sum(arr[y+1, x+1:] * 10**np.arange(arr[y, x+1:].size)[::-1])
  #              else:
                gears.append(np.sum(arr[y+1, x+1:] * 10**np.arange(arr[y, x+1:].size)[::-1]))
        else:
            left = np.where((arr[y+1,:x]<=-1)[::-1])[0]
            if left.size == 0:
                left = 0
...
#            if starData[y,x]==0:
 #               sum += np.sum(arr[y+1, x-left:x+1+right] * 10**np.arange(lenght)[::-1])
  #          else:
            gears.append(np.sum(arr[y+1, x-left:x+1+right] * 10**np.arange(lenght)[::-1]))
    if x != arr.shape[0]-1: # right
        lenght = np.where(arr[y,x+1:]<=-1)[0]
        if lenght.size != 0:
            lenght = lenght[0]
#            if starData[y,x]==0:
 #               sum += np.sum(arr[y, x+1:x+1+lenght] * 10**np.arange(lenght)[::-1])
  #          else:
            gears.append(np.sum(arr[y, x+1:x+1+lenght] * 10**np.arange(lenght)[::-1]))
        else:
#            if starData[y,x]==0:
 #               sum += np.sum(arr[y, x+1:] * 10**np.arange(arr[y, x+1:].size)[::-1])
  #          else:
            gears.append(np.sum(arr[y, x+1:] * 10**np.arange(arr[y, x+1:].size)[::-1]))
